...
static void *spx_fft_init(int size)
{
...
static void spx_fft_destroy(void *table)
{
   spx_drft_clear(table);
   speex_free(table);
}

static void spx_fft(void *table, float *in, float *out)
{
   if (in==out)
   {
      int i;
...
static void spx_ifft(void *table, float *in, float *out)
{
   if (in==out)
   {
      speex_warning("FFT should not be done in-place");
...
static void spx_fft_float(void *table, float *in, float *out)
{
   int i;
#ifdef USE_SMALLFT
...
static void spx_ifft_float(void *table, float *in, float *out)
{
   int i;
#ifdef USE_SMALLFT
   int N = ((struct drft_lookup *)table)->n;
