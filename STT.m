...
- (id)initWithCustomDisplay:(NSString *)nibName {
    if ((self = [super init])) {
        ...
        if (nibName) {
            sineWave = [[WaveView alloc] initWithNibName:nibName bundle:nil];
...
- (void)reset {
    ...
    sineWave.points = volumeDataPoints;

if ([delegate respondsToSelector:@selector(updateDataPoints:)]) {
    [delegate updateDataPoints:volumeDataPoints];
}
}
...
- (void)checkMeter {
    ...
    if ([delegate respondsToSelector:@selector(updateWaveDisplay)]) {
        [delegate updateWaveDisplay];
    }
...
- (void)postByteData:(NSData *)byteData {
    ...
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"https://www.google.com/speech-api/v1/recognize?xjerr=1&client=chromium&lang=%@",kLang]];
